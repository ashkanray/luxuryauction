...
def callback(ch, method, properties, body):
    ...
    if notification_type == "auction_end":
        ...
        status = "completed"  # or whatever status you use for completed auctions
        purchase_date = message['purchase_date']  # Format this as your API expects
        ...
        response = requests.post(f"http://{host_ip}:3901/api/item/add_purchase", json=purchase_data)
        print(response.text)
